...
cut=5000#分训练集测试集 最后cut个样本为测试集

train = result[:-cut, :]
x_train = train[:, :-1]
#y_train = train[:, -1][:,-1]
y_train = df1["二次风和"][:-cut-21]
...
model.add(GRU(32, input_shape=(window, feanum), return_sequences=True))#建立LSTM层
...
print ('R Squared =',r2_score(y_train_predict, y_train))

print('测试集上的MAE/MSE/MAPE')
print(mean_absolute_error(y_test_predict, y_test))
print(mean_squared_error(y_test_predict, y_test) )
print(mape(y_test_predict, y_test) )
print ('R Squared =',r2_score(y_test_predict, y_test))